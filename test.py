...
input_size = 9
motif_size = 3
hidden_sizes = [6, 3]  # 确保隐藏层的大小是 motif_size 的倍数
output_size = 2

nn = MotifBasedSparseNN(input_size, motif_size, hidden_sizes, output_size, activation_fn=Relu(), loss_fn=CrossEntropy())
X = np.random.rand(5, input_size)  # 5个样本，每个样本有9个特征
y_true = np.random.randint(0, 2, (5, output_size))  # 5个样本，每个样本有2个类别，模拟one-hot编码

# 训练神经网络
nn.train(X, y_true, epochs=100, learning_rate=0.01)

# 预测
_, A_list = nn.forward(X)
print("预测值:", A_list[-1])
