...
def train_fashion_mnist(no_training_samples=5000, no_testing_samples=1000, motif_size=4, hidden_sizes=[ 3000, 3000, 3000], epochs=30, learning_rate=0.05, batch_size=40, zeta=0.1):
    # 数据准备
    ...
    # 初始化和训练模型
    nn = MotifBasedSparseNN(input_size, motif_size, hidden_sizes, output_size, init_network='uniform', epsilon=20, activation_fn=Relu(), loss_fn=CrossEntropy())
    ...
    for epoch in range(epochs):
        ...
        # 打印每个epoch的信息
        print(f"Epoch {epoch + 1}: Accuracy: {current_accuracy:.4f}, Time: {epoch_end_time - epoch_start_time:.2f} seconds")
